...
class WeightsParser:
	...
	def __init__(self, filename):
		...
		with open(filename, 'r') as weights_file:
			...
			self.std = self.__parse_line(weights_file.readline().strip(), self.__FEATURE_COUNT)
			self.w = []
			for _ in range(self.__MODEL_COUNT):
				self.w.append(self.__parse_line(weights_file.readline().strip(), self.__FEATURE_COUNT))
