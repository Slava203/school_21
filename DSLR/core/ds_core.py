...
class DataAnalyzer:
    ...
    def describe(self):
        ...
        print()
        # for i in list(set(range(len(self.headers))) - set(self.num_fichas)):
        #     print(f'{self.headers[i]:15.15}', end=' |')
        #     print(f'{count_(self.data[:, i]):>12.4f}', end=' |')
    ...
    def plot_histograms(self):
        ...
        for ficha_name in distribute_per_house.keys():
            fig, ax = plt.subplots()
            self.__generate_histogram(ax, ficha_name=ficha_name, ficha_data=distribute_per_house[ficha_name])
            filename = self.__HISTOGRAM_DIRECTORY + '%s' % (ficha_name) + '.png'
            fig.savefig(filename)
            print('Saved histogram at ' + Fore.BLUE + filename + Fore.RESET)

    def  __scatter_plot(self, ax, ficha_name_1, ficha_name_2, ficha_data_1, ficha_data_2, figsize=None):
        if set(ficha_data_1.keys()) != set(ficha_data_2.keys()):
            raise Exception(Style.BRIGHT + Fore.RED + "Different count of fuculte")

        for faculte_name in ficha_data_1.keys():
            ax.scatter(ficha_data_1[faculte_name], ficha_data_2[faculte_name],  label=faculte_name,
        ...
        for ficha_name_1 in distribute_per_house.keys():
            for ficha_name_2 in distribute_per_house.keys():
                if ficha_name_1 != ficha_name_2:
                    fig, ax = plt.subplots()
                    self.__scatter_plot(ax, ficha_name_1=ficha_name_1, ficha_name_2=ficha_name_2, ficha_data_1=distribute_per_house[ficha_name_1], ficha_data_2=distribute_per_house[ficha_name_2])
                    filename = self.__SCATTERPLOT_DIRECTORY + '%s - %s' % (ficha_name_1, ficha_name_2) + '.png'
                    fig.savefig(filename)
                    print('Saved scatter_plot at ' + Fore.BLUE + filename + Fore.RESET)
    ...
    def plot_pair_plot(self):
        ...
        for i, ficha_name_1 in enumerate(distribute_per_house.keys()):
            for j, ficha_name_2 in enumerate(distribute_per_house.keys()):
                if ficha_name_1 != ficha_name_2:
                    self.__scatter_plot(ax[i, j], ficha_name_1=ficha_name_1, ficha_name_2=ficha_name_2,
                                        ficha_data_1=distribute_per_house[ficha_name_1],
                                        ficha_data_2=distribute_per_house[ficha_name_2])
                else:
                    self.__generate_histogram(ax[i, j], ficha_name=ficha_name_1, ficha_data=distribute_per_house[ficha_name_1])

        filename = self.__PAIRPLOT_DIRECTORY + 'pair_ploy.png'
        fig.savefig(filename)
