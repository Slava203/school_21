...
/*   Updated: 2019/09/26 14:14:06 by daron            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int default_color(int x, t_map *map)
{
	...
	red = get_light((FIGURE_START >> 16) & 0xFF, (FIGURE_END >> 16) & 0xFF, percent);
	green = get_light((FIGURE_START >> 8) & 0xFF, (FIGURE_END >> 8) & 0xFF, percent);
	blue = get_light(FIGURE_START & 0xFF, FIGURE_END & 0xFF, percent);
...
int	get_color(t_point cur, t_point start, t_point end, t_point delta)
{
	...
	red = get_light((start.collor >> 16) & 0xFF, (end.collor >> 16) & 0xFF, percent);
	green = get_light((start.collor >> 8) & 0xFF, (end.collor >> 8) & 0xFF, percent);
	blue = get_light(start.collor & 0xFF, end.collor & 0xFF, percent);
	return ((red << 16) | (green << 8) | blue);
}

/*
 ** Возвращает число из отрезка [p1, p2]
*/
